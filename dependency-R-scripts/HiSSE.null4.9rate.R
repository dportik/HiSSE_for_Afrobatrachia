#### This is a version of the hisse.null4 function from the HiSSE package (Beaulieu and O'Meara 2016) that has been modified by Sean Harrington ######
### To run this with 9 transition rates, set trans.type = "All.no.dual"
hisse.null4.mod.9.rates<-function (phy, data, f = c(1, 1), turnover.anc = rep(c(1, 2, 
                                                       3, 4), 2), eps.anc = rep(c(1, 2, 3, 4), 2), trans.type = "equal", 
          condition.on.survival = TRUE, root.type = "madfitz", root.p = NULL, 
          output.type = "turnover", sann = FALSE, sann.its = 10000, 
          max.tol = .Machine$double.eps^0.25) 
{
  phy$node.label <- NULL
  ...
  if (trans.type == "equal") {
    ...
    trans.mat <- rbind(cbind(sub.mat1, sub.mat3), cbind(sub.mat4, 
                                                        sub.mat2))
    trans.mat[!is.na(trans.mat)] = 1
    rownames(trans.mat) <- c("(0A)", "(0B)", "(0C)", "(0D)", 
                             "(1A)", "(1B)", "(1C)", "(1D)")
    colnames(trans.mat) <- c("(0A)", "(0B)", "(0C)", "(0D)", 
                             "(1A)", "(1B)", "(1C)", "(1D)")
  }
  
  
#### here to next comments is where I have added in my own matrix that allows different transition rates (Sean Harrington)
  if (trans.type == "All.no.dual") {
    
    trans.mat<-matrix(c(NA, 1, 2,2,3,NA,NA,NA,1,NA,2,2,NA,3,NA,NA,4,4,NA,1,NA,NA,5,NA,4,4,1,NA,NA,NA,NA,5,6,NA,NA,NA,NA,1,7,7,NA,6,NA,NA,1,NA,7,7,NA,NA,8,NA,9,9,NA,1,NA,NA,NA,8,9,9,1,NA),8,8)
...
##### end my contribution (Sean Harrington)
  else {    
  ...
  pars = c(turnover.anc)
  eps.anc.tmp = eps.anc
  eps.anc.tmp[which(eps.anc.tmp > 0)] = (eps.anc.tmp[which(eps.anc.tmp > 
                                                             0)] + max(pars))
  pars = c(pars, eps.anc.tmp)
  trans.rate.tmp = trans.rate[!is.na(trans.rate)]
  trans.rate.tmp[which(trans.rate.tmp > 0)] = (trans.rate.tmp[which(trans.rate.tmp > 
                                                                      0)] + max(pars))
  pars = c(pars, trans.rate.tmp)
  np <- max(pars)
  pars[pars == 0] <- np + 1
  cat("Initializing...", "\n")
  data.new <- data.frame(data[, 2], data[, 2], row.names = data[, 
                                                                1])
  ...
  if (length(f) == 2) {
    ...
    if (length(f) == Ntip(phy)) {
      samp.freq.tree <- Ntip(phy)/sum(table(data.new[, 
                                                     1])/mean(f))
    }
    else {
  ...
  if (sum(eps.anc) == 0) {
    init.pars <- starting.point.generator(phy, 2, samp.freq.tree, 
                                          yule = TRUE)
    names(init.pars) <- NULL
    def.set.pars <- c(rep(log(init.pars[1] + init.pars[3]), 
                          8), rep(log(init.pars[3]/init.pars[1]), 8), rep(log(init.pars[5]), 
                                                                          32))
  }
  else {
    init.pars <- starting.point.generator(phy, 2, samp.freq.tree, 
    ...
    def.set.pars <- c(rep(log(init.pars[1] + init.pars[3]), 
                          8), rep(log(init.eps), 8), rep(log(init.pars[5]), 
                                                         32))
  }
  np.sequence <- 1:np
  ip <- numeric(np)
  ...
  if (sann == FALSE) {
    ...
    out = subplex(ip, fn = DevOptimizeNull, control = list(reltol = max.tol, 
                                                           parscale = rep(0.1, length(ip))), pars = pars, phy = phy, 
                  data = data.new[, 1], f = f, condition.on.survival = condition.on.survival, 
                  root.type = root.type, root.p = root.p, np = np)
  }
  else {
    cat("Finished. Beginning simulated annealing...", "\n")
    out.sann = GenSA(ip, fn = DevOptimizeNull, lower = lower, 
                     upper = upper, control = list(max.call = sann.its), 
                     pars = pars, phy = phy, data = data.new[, 1], f = f, 
                     condition.on.survival = condition.on.survival, root.type = root.type, 
                     root.p = root.p, np = np)
    cat("Finished. Refining using subplex routine...", "\n")
    out = subplex(out.sann$par, fn = DevOptimizeNull, control = list(reltol = max.tol, 
                                                                     parscale = rep(0.1, length(ip))), pars = pars, phy = phy, 
                  data = data.new[, 1], f = f, condition.on.survival = condition.on.survival, 
                  root.type = root.type, root.p = root.p, np = np)
  }
  ...
  obj = list(loglik = loglik, AIC = -2 * loglik + 2 * np, AICc = -2 * 
               loglik + (2 * np * (Ntip(phy)/(Ntip(phy) - np - 1))), 
             solution = solution, index.par = pars, f = f, condition.on.survival = condition.on.survival, 
             root.type = root.type, root.p = root.p, phy = phy, data = data, 
             output.type = output.type, trans.type = trans.type, trans.mat = trans.mat, 
             max.tol = max.tol)
